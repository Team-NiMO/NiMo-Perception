...
from stalk_detect.config import WORLD_FRAME
...
class Visualizer:
    ...
    def __init__(cls):
        ...
        cls.data_to_ros_type = {
            ...
            o3d.geometry.PointCloud: (PointCloud2, lambda pcl: orh.o3dpc_to_rospc(pcl, frame_id=WORLD_FRAME, stamp=rospy.Time.now())),
    ...
    def _point_to_marker(cls, point: Point, color, id: int, size: float) -> Marker:
        ...
        Returns
            visualization_msgs.msg.Marker: the marker
        '''
        header = Header(frame_id=WORLD_FRAME, stamp=rospy.Time.now())
    ...
    def publish_item(cls, topic, item, **kwargs):
        ...
        if type(item) in cls.data_to_ros_type.keys():
            ...
            msg = item
            msg.header = Header(frame_id=WORLD_FRAME, stamp=rospy.Time.now())
